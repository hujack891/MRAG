...
def main():
    ...
    INPUT_DIR = "./data/doc_bk"  
    OUTPUT_DIR = "./index/text/v2"
    ...
    for i in range(len(all_chunk_data)):
        if all_chunk_data[i].content_type == "one":
            prompt = (
                f"This excerpt is taken from the file \"{all_chunk_data[i].source_file}\", "
                f"under the primary heading \"{all_chunk_data[i].h1_title}\". "
                f"The content is presented below:\n\n"
                f"{all_chunk_data[i].content}"
            )
        elif all_chunk_data[i].content_type == "two":
            prompt = (
                f"This excerpt is taken from the file \"{all_chunk_data[i].source_file}\", "
                f"within the section \"{all_chunk_data[i].h1_title}\" "
                f"and the sub-section \"{all_chunk_data[i].h2_title}\". "
                f"The content is presented below:\n\n"
                f"{all_chunk_data[i].content}"
            )
