...
def main():
    ...
    OUTPUT_DIR = "./index/img_summary/v1"
    ...
    logger.info(f"成功读取{len(all_summary_data)}个chunk")
    ...
    def process_chunk(index, chunk_data):
        try:
            # 构建 messages 内容
            prompt_text = build_prompt_text(chunk_data)
            messages = [
                {
                    "role": "user",
                    "content": [
            ...
            chunk_data.summary_promot = messages

            chunk_hash = hashlib.md5(json.dumps(messages, ensure_ascii=False).encode('utf-8')).hexdigest()
    ...
    with ThreadPoolExecutor(max_workers=20) as executor:
        futures = [executor.submit(process_chunk, i, chunk) for i, chunk in enumerate(all_summary_data)]
