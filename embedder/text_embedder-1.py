...
class EmbeddingData:
    chunk_id: int
    source_file: str
    content_type: str        # Fixed as "paragraph"
    h1_title: str            
    h2_title: str 
    h3_title: str             
    content: str             
...
def main():
    ...
    for i in range(len(all_chunk_data)):
        chunk = all_chunk_data[i]
        if chunk.h3_title:
            prompt = (
                f"This is an excerpt from the file '{chunk.source_file}', "
                f"within the main section titled '{chunk.h1_title}', "
                f"specifically from the subsection '{chunk.h2_title}' "
                f"and the sub-subsection '{chunk.h3_title}'.\n"
